...
%   $Revision: 1.1.6.11 $  $Date: 2009/02/10 20:56:01 $
...
while ~end_of_file
   ...
   case 'cue'
      ...
      [opt_ck,msg] = read_cueck(fid, ck, opt_ck);
      if ~isempty(msg)
...
% ---------------------------------------------
% READ_CUECK: Read the CUE chunk:
% ---------------------------------------------
function [opt_ck,msg] = read_cueck(fid,ck,orig_opt_ck)

opt_ck      = orig_opt_ck;
orig_pos    = ftell(fid);
total_bytes = ck.Size; % # bytes in subchunk
nbytes      = 4;       % # of required bytes in <cue-ck> header
msg = '';
err_msg = 'MATLAB:audiovideo:wavread:ErrorReadingChunk';
...
% A CUE Chunk is comprized of two items
%   Num Cue Points - This value specifies the number of following cue 
%                       points in this chunk.
%   List of Cue Points - A list of cue points is simply a set of 
%                        consecutive cue point descriptions that follow the
...
% Read standard <cue-ck> data:
opt_ck.cues = fread(fid,1,'uint32');

for cue = 1:opt_ck.cues

    opt_ck.cue_ID(cue) = fread(fid,1,'uint32');
    opt_ck.cue_Position(cue) = fread(fid,1,'uint32');
    opt_ck.cue_ckID(cue) = fread(fid,1,'uint32');
    opt_ck.cue_ckstart(cue) = fread(fid,1,'uint32');
    opt_ck.cue_bkstart(cue) = fread(fid,1,'uint32');
    opt_ck.cue_sampleoffset(cue) = fread(fid,1,'uint32');
end
